    ...
    return (null == resInfo) ? (<Shimmer />) : (
        <div className="container m-auto py-2 md:px-12 lg:px-7">
            ...
            <div className="resturant-menu-accordion rounded-lg my-5">
                {
                    categories.map((categoryData, index) => (<RestaurantMenuAccordion key={categoryData?.card?.card.title} data={categoryData?.card?.card} showIndex={index === showIndex ? true : false} setShowIndex={() => setShowIndex(index)} />))
                }
            </div>
        </div>
    )
